...
class UIFE(nn.Module):
    ...
    classifier_cfg = dict(
        backbone=dict(
            ...
            style='pytorch',
            type='ResNet',
            init_cfg=dict(type='Pretrained', checkpoint=r'ckpt/resnet34.pth')
        ),
        head=dict(
            in_channels=512,
    ...
    def __init__(self, depth=2, width=24, n_heads=6, norm_layer=nn.InstanceNorm2d, act_layer=nn.ReLU):
        super().__init__()

        self.depth = depth
        self.classifier = build_classifier(self.classifier_cfg)
...
class OursPlusOther(BaseModule):
    def __init__(self, **kwargs):
        super().__init__()

        self.pre = UIFE(depth=4, width=24, n_heads=6, norm_layer=nn.InstanceNorm2d, act_layer=nn.ReLU)
        self.net = MODELS.build(kwargs['other'])
